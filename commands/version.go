...
const (
	repoUrl     = "https://github.com/kform-tools/kformpkg"
	downloadURL = "https://github.com/kform-tools/kformpkg/raw/main/install.sh"
)

func GetVersionCommand(ctx context.Context) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "version",
		Short: "show kformpkg version",
...
func GetVersionUpgradeCommand(ctx context.Context) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "upgrade",
		Short: "upgrade kformpkg to latest available version",
		//PreRunE: sudoCheck,
		RunE: func(cmd *cobra.Command, args []string) error {
			f, err := os.CreateTemp("", "kformpkg")
